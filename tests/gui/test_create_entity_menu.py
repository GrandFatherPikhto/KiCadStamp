...
`plan_2026_09_20_empty_cluster_normalisation.md`. Обе стороны одного и того же
токена в CreateEntityDialog.result_data(): пустое поле обязано выйти как None
(С1/С2), а НАБРАННОЕ — доехать как есть, с обрезанными пробелами по краям
(С5/С6); сверх того — сквозной путь через НАСТОЯЩУЮ форму (С3) и путь имени
(С4). Номера там СВОИ, снова С1–С6, поэтому функции названы с префиксом
normalisation_ — в прогоне `pytest -k c1` не имеют права смешиваться два
разных «С1» из двух разных планов. Причина, по которой этот класс дыр вообще
возник: все сторожа выше подают значения МИМО формы (подменённый
CreateEntityDialog), а нормализация живёт именно в ней — проверено правило, не
проверен путь к нему.
"""
import sys
...
# ═══════════════════════════════════════════════════════════════════════════
# Обратное направление на уровне формы — тот же план, §1 Т4 (С5/С6, М5/М6)
# ═══════════════════════════════════════════════════════════════════════════
#
# ЗАЧЕМ. С1/С2 держат ровно ОДНО направление: пустое поле обязано стать None.
# Обратное — «набранное доезжает до записи» — приёмка Т1 замерила мутацией (всё
# выражение кластера заменено на безусловное `cluster = None`, то есть форма
# молча выбрасывает то, что человек набрал): прогон остался ПОЛНОСТЬЮ зелёным,
# 5539 passed. Ни один сторож потери не замечает — потому что С3 держит пустое
# поле, а сквозной путь с НАБРАННЫМ значением не проходится ни разу.
#
# Последствие такой потери — тот же исход, что и у дыры §0 плана, только с
# другой стороны: сущность ложится БЕЗ кластера, рядом с размеченной, и
# клонировать её по кластеру уже нельзя — а никто об этом не скажет.
#
# Читается форма РОВНО ТАК ЖЕ, как в С1/С2 — через её собственный
# result_data(), не мимо неё. Случай " CH0 " (значение с пробелами по краям) —
# первый сторож на .strip() при НЕПУСТОМ значении: до него .strip() держали
# только случаи из одних пробелов (мутации М1/М2 роняли по одному случаю
# [   ] и ничего больше). Две половины — ячейка и отпечаток — отдельными
# функциями: падение одной не прячет состояние другой (приём С5а/С5б).

@pytest.mark.parametrize("typed,expected", [("CH0", "CH0"), (" CH0 ", "CH0")])
def test_normalisation_c5_a_typed_cluster_on_a_cell_reaches_result_data(
        main_window, typed, expected):
    """С5 (§1 Т4): человек НАБРАЛ кластер у ячейки — он обязан выйти из формы
    как есть, а с пробелами по краям — обрезанным, но не выброшенным.
...
def test_normalisation_c6_a_typed_sheet_on_an_imprint_reaches_result_data(
        main_window, typed, expected):
    """С6 (§1 Т4), вторая половина: у отпечатка набранный ЛИСТ обязан доехать —
    это целевой лист twin-резолва (config/models.py:704), и потеря его значит
    сущность клонируется не на тот лист.

    Отдельной функцией — падение половины про ячейку не должно прятать эту.
    Роняют её те же М5л/М6л по полю листа: безусловное `sheet = None` и
    снятый .strip().
    """
    dlg = _RealCreateEntityDialog(main_window, "imprint", "amp", [])
    assert dlg._cluster_edit is None, (
    ...
    assert sheet == expected, (
        ...
        "`sheet = None`), а пробелы по краям — обрезаться (мутация М6л: "
        f"снятый .strip()); набрано {typed!r}, получили {sheet!r}, "
        f"ждали {expected!r}")


# ═══════════════════════════════════════════════════════════════════════════
# Оставшиеся две ветки того же токена — план typed_value_survives_the_form,
# §1 Т1/Т2 (сторожа С7/С8, мутации М7/М8/М9/М10)
# ═══════════════════════════════════════════════════════════════════════════
#
# ПОЧЕМУ ЭТО ОТДЕЛЬНЫЕ ФУНКЦИИ, А НЕ ТРЕТИЙ ПАРАМЕТР У С5/С6. С5 держит
# кластер (поле есть только у ячейки), С6 — лист у ОТПЕЧАТКА. Между ними
# остались ровно две двери, через которые значение печатают, а сторожей нет:
#
#   * лист у ЯЧЕЙКИ. В форме это третье поле, а в записи лист у ячейки — не
#     «второе ключевое поле пары», как у отпечатка, а СУЖЕНИЕ РОЛИ
#     (build_role_anchor / anchor_sheet). Пишется он ОБЕИМ ветвям безусловно:
#     gui/dock_hub.py:2962 — `if sheet:` без оглядки на source_field. Прежний
#     сквозной сторож заполненного листа (extra_filled_sheet...reaches_the_record)
#     идёт по ветке ОТПЕЧАТКА — то есть ячейковая ветка не проверена ни разу, и
#     мутация «обнулять лист только у ячейки» ложится ровно в эту дыру.
#   * ИМЯ. У него нет токена `or None`: `name = self._name_edit.text().strip()`.
#     Но .strip() там работал ровно тогда, когда ОБНУЛЯЛ (пустое из одних
#     пробелов — случай сторожа С4), а на набранном — нет.
#
# Обе дыры найдены МУТАЦИЯМИ, а не рассуждением: при каждой из них прогон этого
# файла оставался ПОЛНОСТЬЮ зелёным (35 passed). Отдельные функции, а не общий
# параметр, — приём С5а/С5б: падение половины про отпечаток не должно прятать
# половину про ячейку, именно на этой асимметрии дыра и выжила.


@pytest.mark.parametrize(
    "typed,expected", [("Channel_1", "Channel_1"), (" Channel_1 ", "Channel_1")])
def test_normalisation_c7_a_typed_sheet_on_a_cell_reaches_result_data(
        main_window, typed, expected):
    """С7 (§1 Т1 плана typed_value_survives_the_form): человек НАБРАЛ лист у
    ЯЧЕЙКИ — он обязан выйти из формы как есть, а с пробелами по краям —
    обрезанным, но не выброшенным.

    Последствие потери здесь тише, чем у отпечатка (там сущность клонируется не
    на тот лист), и потому опаснее: у ячейки лист сужает резолв роли, а роль с
    потерянным листом не падает — она начинает разрешаться неоднозначно, молча и
    позже. Сторож читает ровно выход result_data(), как С1/С2/С5/С6: ни через
    хаб, ни по внутренним полям формы.

    Отдельной функцией от С6 (приём С5а/С5б): у С6 ветка отпечатка, здесь —
    ячейки, и мутация одной ветки обязана НЕ ронять половину про другую.

    Роняют сторож обе правки, оставляющие сегодня прогон зелёным:
...
def test_normalisation_c8_a_typed_name_reaches_result_data_trimmed(
        main_window, typed, expected):
    """С8 (§1 Т2): набранное ИМЯ обязано доехать из формы ОБРЕЗАННЫМ — и
    ...
    два разных имени. Пробелы по краям превращают попытку создать дубликат во
    вторую сущность с тем же по смыслу именем, и узнает об этом только тот, кто
    потом откроет файл.

    .strip() при ПУСТОМ имени держит С4 («имя из одних пробелов») — поэтому
    сторож стоит рядом с ним, а не вместо него: М9 обязана уронить ИМЕННО ЭТУ
    функцию и оставить С4 зелёным, и этим доказывается, что проверено ровно то,
    чего С4 не видел.
    ...
    Роняют сторож две правки:
      ...
      * М10 — .strip() снят у имени целиком (контроль: красны и С8, и С4).
    """
    dlg = _RealCreateEntityDialog(main_window, "cell", "seed_cell", [])
