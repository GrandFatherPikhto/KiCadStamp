...
# ── New: File > "Create Project..." (2026-09-24) ───────────────────────────────
#
# These guards used to mock QFileDialog.getSaveFileName: the old "New Root file..."
# flow created an EMPTY FILE wherever the save dialog pointed. Denis replaced that
# twice on 2026-09-24 — first with "a project is created by picking a DIRECTORY",
# then with "В File будет «Создать проект». Открываем диалог создания проекта. Там
# указываем имя, как в кикад, автоматически создаётся директория с нужной
# инфраструктурой". The dock now only OPENS that dialog and opens what it reports:
# the form, the validation and the refusal belong to the widget, and the writing to
# kicadstamp.project_setup.
#
# Each old guard keeps its property, at the layer that now owns it:
#   * "creates it and opens it" -> HERE (the dock opens the created project);
#   * "refuses an existing one" -> the WIDGET, and HERE only the dock half: a
#     refusal must not switch the open project;
#   * "cancel leaves it alone"  -> HERE, unchanged;
#   * "the canonical template"  -> the WIDGET and kicadstamp.project_setup;
#   * "only the config file"    -> FLIPPED into "the config AND the five
#     infrastructure directories" by Denis's 2026-09-24 decision, which consciously
#     overrode the 2026-09-11 rule it stood on; owned by
#     tests/test_project_is_a_directory.py (core) and
#     tests/gui/test_create_project_dialog.py (end to end through the widget).
#
# The dialog is exercised FOR REAL below — only exec() is replaced, because a modal
# event loop is what a test must never enter — so these cells fail if the dock
# stops passing a usable start_dir or stops opening the reported path.


def _drive_create_dialog(monkeypatch, name, folder) -> list:
    """Make the CreateProjectDialog the dock builds behave as if the user typed
    `name` in `folder` and pressed OK. Returns the `start_dir` values the dock
    passed, so the starting-folder property is assertable too.

    exec() returns self.result(), exactly as the real modal does — so a REFUSED
    dialog reports Rejected and the dock must then leave the open project alone."""
    real = create_project_dialog_mod.CreateProjectDialog
    seen: list = []

    def build(parent=None, start_dir=""):
        seen.append(start_dir)
        dialog = real(parent, start_dir=start_dir)
        dialog.name_edit.setText(name)
        dialog.folder_edit.setText(str(folder))
        dialog.exec = lambda: (dialog._on_ok(), dialog.result())[1]
        return dialog

    monkeypatch.setattr(root_metadata_mod, "CreateProjectDialog", build)
    return seen


def test_new_root_creates_the_project_and_opens_it(main_window, tmp_path, monkeypatch):
    """Н1 (plan_2026_09_24_project_is_a_directory, second step): a name plus a
    folder produce <folder>/<name>/, and the dock opens <folder>/<name>/<name>.sexp
    — recent list included."""
    dock = RootMetadataDock(main_window)
    seen = _drive_create_dialog(monkeypatch, "HiPiMS-v099", tmp_path)

    dock._on_new_root()

    created = tmp_path / "HiPiMS-v099" / "HiPiMS-v099.sexp"
    ...
    # No project was open, so the dialog had no folder to start from.
    assert seen == [""]


def test_new_root_starts_the_dialog_beside_the_open_project(
        main_window, tmp_path, monkeypatch):
    """The dialog opens where PROJECTS live, not inside the current one: for a
    project at <parent>/<name>/<name>.sexp the start folder is <parent>, so the
    second project lands next to the first."""
    project = tmp_path / "first" / "First"
    project.mkdir(parents=True)
    root = project / "First.sexp"
    _write(root, MINIMAL_CELL)
    dock = RootMetadataDock(main_window)
    dock.set_root_file(root)

    seen = _drive_create_dialog(monkeypatch, "Second", project.parent)
    dock._on_new_root()

    assert seen == [str(project.parent)]


def test_new_root_refused_by_the_dialog_leaves_the_project_alone(
        main_window, tmp_path, monkeypatch):
    """Н4's dock half. The refusal itself — a warning box, ONE ERROR line, nothing
    written — is the widget's cell (tests/gui/test_create_project_dialog.py). What
    matters HERE is that a refused dialog does not switch the open project: the dock
    opens only what the dialog actually created."""
    taken = tmp_path / "first" / "Taken"
    taken.mkdir(parents=True)
    existing = taken / "Taken.sexp"
    _write(existing, MINIMAL_CELL)
    before = existing.read_text(encoding="utf-8")

    root = tmp_path / "open_project.sexp"
    _write(root, MINIMAL_CELL)
    dock = RootMetadataDock(main_window)
    dock.set_root_file(root)

    _drive_create_dialog(monkeypatch, "Taken", taken.parent)
...
def test_new_root_dialog_cancelled_leaves_root_untouched(main_window, tmp_path, monkeypatch):
    ...
    class _Rejected:
        """A cancelled CreateProjectDialog: exec() returns Rejected and reports
        nothing created."""

        def __init__(self, parent=None, start_dir=""):
            self.created_config_path = None

        def exec(self):
            return 0

    monkeypatch.setattr(root_metadata_mod, "CreateProjectDialog", _Rejected)
    dock._on_new_root()

    assert dock._path == root


# The two guards that stood here are GONE from this file on 2026-09-24, and neither
# was weakened:
#   * "a brand-new .sexp starts as the (kicadstamp-config) template" — asserted
#     where the template is WRITTEN: tests/test_project_is_a_directory.py
#     (create_project) and tests/gui/test_create_project_dialog.py (end to end);
#   * "only the config file, no infrastructure directories" — FLIPPED by Denis's
#     2026-09-24 decision ("автоматически создаётся директория с нужной
...
def test_creates_actions_with_stable_ids_and_defaults(main_window):
    ...
    expected = {
        ...
        ACTION_NEW: ("Create Project...", "Ctrl+N"),
