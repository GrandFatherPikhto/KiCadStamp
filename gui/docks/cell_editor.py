...
Anchor (added 2026-08-06; reshaped by plan_2026_09_09_cell_anchor_v2) —
...
consumer reads. This form now offers only the OFFLINE Role anchor
(anchor_role, optional anchor_pad = the component-centre case); anchor_role
is a closed combo sourced from THIS cell's own current Components list (not
the live board) — it must name one of them (validated on Save, see
config/entries.py's _load_cell). A STORED anchor_xy is NOT edited here: it is
carried through verbatim on save (never dropped — GUARD 1/2, an explicit
anchor_xy wins over Role/Pad), and it is edited in the dedicated "Cell
anchor" page (gui/docks/cell_anchor_view.py: Role+Pad resolved live at apply
time, or an explicit marker point). The old "XY" mode and the "Take
coordinates from selection" button — which COMPUTED a numeric anchor_xy at
save time — were removed (Фаза B): §0.1 of that plan established that a
Role+Pad anchor is a REFERENCE resolved at apply time (resolve_pad_mount),
never a number stored at save time. The anchor combo is keyed by
currentData() strings ("none"/"role"), so the item ORDER can never silently
break a reader again (see _refresh_origin_role).
...
class CellDock(QWidget):
    ...
    def __init__(self, main_window):
        ...
        # String keys via currentData(), NOT index (Фаза B): dropping the old
        # "XY" item shifts "Role" from index 2 to 1, and index-based readers
        # (_refresh_origin_role) then silently return None for every
        # Role-anchored cell — build_refresh_plan loses origin_role and falls
        # back to the legacy zero-slot origin. Data keys make the item ORDER
        # irrelevant forever.
        self.anchor_mode_combo = QComboBox()
        self.anchor_mode_combo.addItem(_("(none)"), "none")
        self.anchor_mode_combo.addItem(_("Role"), "role")
        ...
        # The explicit-XY anchor editor and the "Take coordinates from
        # selection" button were REMOVED (Фаза B, plan v2 §B.1): computing a
        # numeric anchor at save time is exactly the mechanism §0.1 rejected.
        # A cell's anchor_xy is now edited in the dedicated "Cell anchor" page
        # (gui/docks/cell_anchor_view.py, Фаза C); this form only keeps the
        # offline Role anchor (component centre).
    ...
    def _on_anchor_mode_changed(self) -> None:
        # currentData(), not currentIndex() — the item order carries no meaning
    ...
    def _refresh_origin_role(self) -> str | None:
        ...
        keeps the legacy zero-slot origin for un-anchored cells.

        Reads currentData() — an index-based check silently broke the moment
        the "XY" item was removed (Фаза B): "Role" moved from index 2 to 1 and
    ...
    def load_entry(self, name: str, file_path: Optional[Path] = None) -> None:
        ...
        try:
            ...
            # NOT edited here — it is carried through verbatim on save (GUARD
            # 1/2, see _build_cell_dict) and edited in the dedicated "Cell
            # anchor" page. A Role-anchored cell opens in Role mode; anything
            # else (anchor_xy-only, or no anchor at all) opens in "(none)".
