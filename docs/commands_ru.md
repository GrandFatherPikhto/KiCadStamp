...
| `--rule-net ЛИТЕРАЛ` | Записывает цепь этой via/track как `null` вместо её литерального имени (например, `+3V3`) — при применении via/track ячейки, размещённой ManualSpoke, с `net: null` наследует цепь охватывающего Rule, поэтому ячейка переиспользуется между Rule'ами с разными цепями. Нужен только для ячеек, переиспользуемых через ManualSpoke — ячейкам с `net_from_role` это не нужно. Можно повторять. Фатал, если та же цепь есть и в `--param`/`--net-template`. |
