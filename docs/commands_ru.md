...
**внутреннем** слое (`In1.Cu`…) поменять нельзя: пара для внутреннего слоя
зависит от медного стека платы (сколько слоёв и в каком порядке), а копирование
канала его не читает. Поэтому `--mirror` отказывает на таком слое фаталом, в
котором названы слой и каналы, — тихая копия не на ту сторону выглядела бы как
успешная, а тихий пропуск зеркала собрал бы конструкцию зеркально неверно.
Собственный слой футпринта всегда F/B (внутренней стороны не бывает), так что
для него пара полна. Маппинг цепей: локальные цепи `/Channel_0/...`
