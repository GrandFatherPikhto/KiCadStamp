...
- **по цепям** – ищет компоненты по ожидаемой цепи. Ожидаемая цепь роли (Фаза 2, шаг 2.1): явные
  ...
  общая для всех кандидатов). `net_template` ячейки с неразрешённым `{placeholder}` (нет
  подходящего `params`) НЕ считается явным источником — он уходит в путь живого авто-вывода
