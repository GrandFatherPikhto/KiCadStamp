...
Дерево БЕЗ `(anchor ...)` вообще (2026-08-31, план tree_self_anchor_from_entity) получает АВТО-якорь:
единственный корневой узел `kind "placement"` указывает на Entity, а ЕДИНСТВЕННЫЙ компонент её ячейки
с локальным смещением (0,0) (без `offset_along_mm`/`offset_across_mm` — «нулевой», самоссылающийся
слот, напр. `role "FPGA"`) становится ролью якоря, суженной собственными `sheet`/`cluster` этой
Entity, и резолвится ЖИВЬЁМ ровно как явный `(anchor (role ...))`. Явный `(anchor ...)` всегда имеет
приоритет; авто применяется ТОЛЬКО при отсутствии анкора (буквальный `(anchor (ref "self"))` — это
по-прежнему прежний cycle-fatal). Неоднозначность конфига (0/2+ нулевых слотов, 0/2+ корневых
placement-узлов) — fatal всего прогона, никогда не тихая догадка; живой сбой резолва роли (роль не
найдена/неоднозначна на плате) — обычный per-tree skip.
