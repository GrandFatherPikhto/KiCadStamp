...
5. **Идемпотентность** – пропуск уже существующих via, треков и компонентов, уже стоящих на целевых позициях (через `skip_existing_components` и реестры расстановки для via и треков). С 2026-08-31 позиционный пре-чек ТРЕКОВ (`filter_existing_tracks`, см. `kicadstamp/registry.py`) выполняется в Фазе 3 **безусловно** (не только под `skip_existing_components`): он только пропускает (skip-only, никогда не удаляет/не усыновляет чужую медь), поэтому повторный редрайв одной и той же ячейки через два разных механизма — например legacy `clone_placement` И Entity, материализованную деревом, — больше не накапливает буквальные дубликаты под разными registry-ключами. С 2026-09-08 то же самое для VIA делает позиционный пре-чек via (`filter_existing_vias`) — но в СОБСТВЕННОЙ фазе (Фаза 2, не Фаза 3): via, чья запись реестра по любой причине перестала совпадать с живой платой (потерянный/перезаписанный реестр, прогон, прерванный между созданием via и записью реестра), больше не пересоздаётся на каждом редрайве (на живом профиле так накопилось до 4 буквальных дубликатов на одну via на цепях FPGA-питания).
...
Позиция узла — это то место, куда ложится **МОНТАЖ A** ячейки: точка, которую выводит
`cell_mount_offset` (`kicadstamp/geometry/cell_anchor.py`) — `anchor_xy`, иначе центр компонента
`anchor_role`, иначе хранимый (0,0), — и ровно та же точка, которую `apply_clone_geometry` называет
`origin`. ЛЮБОЕ живое чтение позиции Entity измеряет ИМЕННО эту точку (2026-09-11, задание
plan_2026_09_11_entity_live_position_mount_point): и «Read current position» на узле дерева, и мост
«Создать дерево из выделения» идут через один живой кадр кластера (`_live_cluster_frame`,
`gui/docks/live_position.py`) — поэтому две кнопки теперь дают одинаковые числа. До этой правки мост
экстракта мерил компонент на хранимом (0,0) ячейки — другую точку, отличающуюся на величину |A|
(замер на `pif_oa_n2v5`: 5.53 мм, `anchor_xy -2.258536 -5.046273`), из-за чего узел уезжал на столько же.
Деградированный путь остался для Entity, которую вообще нельзя прочитать с живого кластера (нет `cell`,
нет `cluster` или ячейки нет в конфиге) — и он теперь СООБЩАЕТ о себе в Log, а не молча мерит не ту точку.
...
Дерево БЕЗ `(anchor ...)` вообще, либо с явным `(anchor (self ...))` (2026-09-11, план
tree_self_anchor, задание Д — прежний АВТО-якорь, 2026-08-31 / переработка 2026-09-05) висит на
компоненте, который дерево размещает САМО: субъект привязки — собственный узел `(self (ref "..."))`,
если он назван (узел вида `kind "placement"` этого дерева), иначе Entity единственного корневого узла
`kind "placement"` — а ролью якоря становится MOUNT-роль её ячейки (`anchor_role`; для ещё не
мигрированной legacy-ячейки — единственный компонент на хранимом (0,0), старый «нулевой слот»),
суженная собственными `sheet`/`cluster` этой Entity, и резолвится ЖИВЬЁМ ровно как явный
`(anchor (role ...))` (`(self (pad "..."))` переносит базу на этот пад). ОТСУТСТВИЕ `(anchor ...)`
читается как `(self)` НАВСЕГДА — миграции нет. Неоднозначность конфига (нет mount-роли И 0/2+ нулевых
слотов, или «голый» `(self)` с 0/2+ корневыми placement-узлами) — fatal всего прогона, никогда не
тихая догадка; живой сбой резолва роли (роль не найдена/неоднозначна на плате) — обычный per-tree
skip.
...
материализация узел не обходит; единственный корневой узел `is_self`-дерева — источник его якоря
по построению, и дублем он не считается никогда.
...
- **Слой размещения** – для каждого компонента может быть свой (per‑placement), для `ManualSpoke` используется глобальный `layer` из конфига. При зеркалировании (`mirror`) инвертируется только внешняя пара (`F.Cu` ↔ `B.Cu`); медь на **внутреннем** слое остаётся на своём слое (это план платы, а не сторона конструкции).
