...
- **Клонирование секций** – для многократно повторяющихся шаблонов используйте `clone_placements` (ожидаемые цепи авто-выводятся с живой платы с Фазы 2, шаг 2.1; явные `nets`/`params` остаются опциональными override-ами) и запускайте без выделения; для штучных экземпляров – режим «по выделению» (без `nets` и `params`, или с `by_selection: true`), выделяя компоненты в KiCad перед запуском.
