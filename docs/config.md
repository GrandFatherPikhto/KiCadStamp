...
| `schematic_dir` | string | Folder with the project's `*.kicad_sch` files, for `anchor_sheet` resolution. Relative to this config file's own location, like `registry_path`. Since 2026-09-11 the Project dock no longer writes this key — its **Reload schematic sheets** button fills `schematic_files` instead and CLEARS `schematic_dir` (build_sheet_name_map reads BOTH sources, so a stale folder glob would re-add unreachable sheets). The format still honours it for hand-written configs. |
| `schematic_files` | list of strings | The `.kicad_sch` files to build the `{uuid: Sheetname}` map from (the root sheet plus every sheet reachable from it). Since 2026-09-11 the Project dock's **Reload schematic sheets** button writes the hierarchy walk here. |
...
| `role_cluster_source` | string | `registry` (the default) or `board` — which side the resolver reads Role/Cluster from. `registry` means OUR override store (`<config-dir>/overrides/<config-stem>.fields.json`, written by the "Refs" tab and fieldstool) wins over the live board **always**, not only when the board is empty: a role typed in KiCadStamp takes effect without touching the board or the schematic, and `Update PCB from Schematic` (F8) no longer overwrites the values that were not applied yet. `board` restores the pre-2026-09-18 behaviour exactly — every value read straight from the board. The store is machine-only json next to the copper registries and is never reachable through `include:`. An unknown value is a FATAL, never a silent fallback to the board; an absent key means `registry`, so profiles written before this change keep working unchanged. |
| — | — | **The one checkbox worth knowing about.** The store's key is the SYMBOL uuid from the schematic, so re-linking footprints by reference designator breaks that link. Inside the F8 dialog (*Update PCB from Schematic*), under "Options", there is a checkbox ☐ **"Re-link footprints to schematic symbols based on their reference designators"**. It is **unchecked by default**, and an ordinary F8 does nothing to the store. Ticked DELIBERATELY, it re-links footprints by refdes — the store's records are then orphaned and the roles have to be typed again. It is a conscious action in the dialog, not a background threat: F8 on its own is safe. |
