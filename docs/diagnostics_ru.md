...
├── board_call_timing.py           # Замер каждого вызова адаптера и каждого round trip kipy (библиотека, отдельно не запускается)
...
Измеряют, где сессия GUI на самом деле проводит время внутри адаптера платы — по вызовам и
по потокам. Два шага: записать, потом разобрать.

```bash
python -m kicadstamp.diagnostics.run_gui_with_timing   # поработать по докам как обычно, выйти
python -m kicadstamp.diagnostics.report_board_timing
```

Запись — это monkey patch, ставящийся до старта GUI (`board_call_timing.py`), поэтому ни один
боевой файл не правится и откатывать нечего. Пишется JSON Lines в
`<репозиторий>/diagnostics/board_timing_<pid>.jsonl` (гитигнорится; по одному самодостаточному
объекту в строке, с немедленным сбросом на диск — падение посреди сессии оставляет читаемый
лог). **Данные платы не записываются никогда**: только имена вызовов, длительности,
количество элементов и поток — такой лог можно без опаски приложить к баг-репорту.

Отчёт печатает по каждому методу количество вызовов с медианой/p90/максимумом, долю сессии,
проведённую внутри адаптера, разбивку по потокам, неуспешные вызовы и десять самых медленных
одиночных вызовов.

Две величины стоит уметь читать:

- **разбивка по потокам** отвечает на вопрос «читает ли плату UI-поток» — именно он решает,
  настоящая у нас заморозка окна или теоретическая;
- **самые медленные пакетные вызовы** задают `DEFAULT_TIMEOUT_MS` по данным, а не на глаз.

Замерено на живой плате в 325 футпринтов 13.09.2026: за сессию в 272 с внутри адаптера
проведено **9.1 с (3.3%)**, из них на UI-потоке — **0.2 с (0.07%)**, а самый медленный
одиночный вызов за всю сессию составил **287 мс** против таймаута в 20 000 мс. Именно этот
замер отменил запланированный перевод всех чтений платы на `await`: заморозки, которую он
должен был вылечить, в природе не оказалось.
...
**Назначение:**
Аудит Cell-шаблонов на `net_template` в виде ЗАХАРДКОЖЕННОГО абсолютного пути
сети (`/Sheet/Group/Signal`) у Cell, которые переиспользуются Entity на более
чем одном листе. `net_template` живёт на уровне Cell (никогда не
параметризуется листом), поэтому каждый инстанс переиспользуемой Cell делит
один и тот же литерал — и когда Cell размещается на втором листе (например,
материализуется декларацией `tree_instances:`), резолюция роли для этого
инстанса может найти — и передвинуть — ТОЛЬКО реальный компонент «заблокиро‑
ванного» листа, молча и без ошибки ambiguity. Именно этот паттерн стоял за
живым багом (2026-09-07): сгенерированный `tree_instances`-ом `ch1_dac_buf`
перерисовал реальные bypass/bulk-конденсаторы канала 0 вместо канала 1.
Скрипт READ-ONLY: грузит каждый конфиг через `load_config` и печатает отчёт —
сам он ничего не чинит (замена литерала на `net_from_role`/`net_from_role_pad`
или ретегирование Cluster на схеме — решение владельца данных профиля).

**Запуск:**
```bash
python -m kicadstamp.diagnostics.audit_cell_net_templates <config.sexp> [<ещё config.sexp> ...]
```

**Вывод:**
Человекочитаемый отчёт, сгруппированный по Cell: для каждой роли, чей
`net_template` начинается с `/`, — полный литерал и лист, на который он
«заблокирован». Если находок нет — одна строка
`No hardcoded sheet-specific net_template found.` Код возврата всегда `0`
(диагностика, не CI-гейт).

**Зависимости:**
`kicadstamp.config.load_config` (офлайн — живой KiCad не нужен).
