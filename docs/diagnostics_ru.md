...
`plan_2026_09_21_board_door_enforcement`). Зонд считает, страж отказывает: чтение
`connection.board` с UI-потока **без знака** бросает `UiThreadBoardReadRefused`, в сообщении —
`файл:строка` ВЫЗЫВАЮЩЕГО, а в Лог попадает ОДНА строка на место (зонд, если он включён, попытку
всё равно считает — он идёт первым). Вне его юрисдикции сознательно: значение `None` (это
