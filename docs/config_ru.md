...
> **Два `rules:` на одной сети, потребляющие один и тот же пул `(role, cluster)` — фатал
> валидации** (2026-08-20): пул строится заново на каждое правило без владения/дистанции, поэтому
> какое правило выполнится позже, то молча заберёт компоненты, предназначенные соседу (больше всего
...
- **`anchor_role`** (обязательно) — поле Role якорного футпринта, ищется по
  ...
  Rule/ClonePlacement. Опциональные `anchor_sheet`/`anchor_cluster` сужают
...
- **`tracks`/`vias`** — медь как локальные (along/across) смещения от точки
  ...
  Сеть ВСЕГДА записана явно на каждом элементе (нет охватывающего Rule, из
