...
- **По сетям** (повторяющиеся секции — pi-фильтры, DAC-каналы), когда заданы `params`/`nets`: каждая
  ...
  сужаются каскадом `anchor_sheet` → собственный `Cluster` этого размещения (его `name:` — НЕ
  `anchor_cluster`, тот сужает только якорь, см. «С якорем» выше) → текущее выделение на плате →
  физическая близость к якорю → фатал, в этом порядке — точный каскад см. в докстрингах
