...
- **Снятия снимков каналов** – извлечения всех компонентов, дорожек и переходных отверстий, относящихся к конкретному экземпляру канала (например, `Channel_0`), в читаемый s-expr-файл.
...
├── extract.py            # Основная точка входа – извлечение канала в s-expr
...
**Назначение:**  
Связывает нетлист и плату, строит снимок канала и сериализует его в s-expr. Это главная точка входа для команды `clone-extract`.
...
5. Преобразует снимок и карту близнецов в словарь Python, готовый для s-expr (с округлением координат до 4 знаков).
...
| `extract_channel(net_path, pcb_path, channel, output)` | Основная функция, вызываемая из CLI. Выполняет все шаги и возвращает словарь с результатом. |
...
## Формат выходного s-expr-снимка
...
Его выходные данные (s-expr-снимки) предназначены для **ручного анализа** разработчиком. Команда `clone-plan` автоматизирует следующий шаг — генерацию готового блока `clone_placements:` из снимка без ручного написания `params`/`nets` (Фаза 3).
...
| `--output` | Путь к выходному s-expr-файлу. |
| `-v, --verbose` | Подробный вывод (логирование). |

**Пример вывода:**

```
[Channel_0] футпринтов: 42, сегментов: 156, виа: 38 -> snapshot.sexp
```
