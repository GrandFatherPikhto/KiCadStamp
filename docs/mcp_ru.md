...
| `kicad_raw_move_footprint` | **высокий (raw)** | Перемещение одного футпринта по ref напрямую через kipy; выключен по умолчанию |
...
- Перед каждой сырой записью выполняется **guard идентичности платы**
  (`check_board_identity`) и всегда сообщается подключённая плата — raw-путь не
  является дырой в защите, которая уже есть у apply.
