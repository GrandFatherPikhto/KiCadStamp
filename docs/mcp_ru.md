...
|---|---|---|
| `kicadstamp_get_board_identity` | низкий | Имя платы + проект, которому она принадлежит (имя + каталог) + версия KiCad |
| `kicadstamp_list_footprints` | низкий | ref, Role/Cluster, позиция (мм), поворот, слой; опция `ref_prefix`; отвечает конвертом с подписью платы (ниже) |
| `kicadstamp_get_footprint` | низкий | Футпринт детально: pads (номер/нет/позиция) и сети на них; отвечает конвертом с подписью платы (ниже) |
...
| `kicadstamp_get_items_by_uuid` | низкий | Детализация объектов платы по uuid (tracks/vias/footprints); каждый запрошенный uuid встречается ровно один раз, отсутствующие — `found: false`; отвечает конвертом с подписью платы (ниже) |
...
**Конверт с подписью платы.** Три инструмента, читающие футпринты
(`list_footprints`, `get_footprint`, `get_items_by_uuid`), отвечают так: полезная
нагрузка — под своим ключом, рядом — идентичность платы, которую она описывает:

```
{"board": {"board_name": "...", "project": {"name": "...", "path": "..."}},
 "footprints": [...]}          # либо "footprint": {...} / "items": [...]
```

Конверт появился потому, что неверный ответ был неотличим от верного: один и тот
же `get_footprint("J6")` уверенно отвечал про две разные открытые платы, и выдавал
это только uuid — по координатам ответы совпадали. Конверт подписывает и ПУСТЫЕ
ответы: `footprints: []`, `items: []` или запись с `found: false` всё равно говорят,
какая плата это сказала, ведь ложное «не найдено» читается как законный ответ.
Идентичность читается внутри ТОГО ЖЕ освежения платы, что дало нагрузку (одно
освежение — один момент), и стоит плате НИЧЕГО: оба поля берутся из описателя
документа, который адаптер и так держит, то есть ноль обращений по IPC, и это
измерено (`kicadstamp/diagnostics/probe_project_identity.py`). Версии KiCad в
конверте намеренно НЕТ: `get_version()` — настоящее обращение по IPC, поэтому
платит за неё по-прежнему `kicadstamp_get_board_identity`.
...
Когда инструмент не может выполнить работу, клиент получает фразу — а не стек —
для каждого отказа, с которым пользователь может что-то сделать:

- **KiCad закрыт** (или был закрыт во время вызова): *«KiCad не отвечает —
  вероятно, он не запущен или был закрыт во время вызова. Запустите KiCad с
  открытой платой и повторите попытку…»*, а слова самого kipy остаются деталью.
  Сообщение локализовано (обычный gettext-каталог проекта) и лежит там, откуда
  CLI сможет взять тот же текст позже — один текст, две поверхности.
- **KiCad занят** (в GUI работает незавершённый инструмент: интерактивная
  трассировка, перемещение, размеры): отказ `AS_BUSY` получает длинное
  объяснение проекта — завершите этот инструмент в KiCad (Esc или правый клик →
  Cancel) и вызовите снова; плата не изменена.
- **Линк обрывается посреди вызова**: сервер сносит мёртвый адаптер и повторяет
  вызов **ровно один раз** со свежим. Ни очереди, ни TTL, ни политики ретраев
  сверх этой единственной попытки нет; если не удалась и она, клиент получает то
  же внятное сообщение, что выше.

Всё остальное — настоящий дефект, и он остаётся отличимым: возвращается штатная
краш-обёртка MCP (в её сообщении только имя инструмента, трассировка уходит в лог
сервера), поэтому баг никогда не подаётся модели как пользовательский отказ.

**Сколько стоит вызов.** Каждый вызов инструмента перестраивает плату перед
ответом, поэтому чтение ЖИВОЕ, а не из кэша, набранного при первом подключении к
KiCad. Замерено на тестовых платах: вызовы, читающие футпринты (например
`kicadstamp_get_footprint`, `kicadstamp_list_footprints`,
`kicadstamp_get_items_by_uuid`), платят ОДНО полное чтение платы — 26–38 мс на
плате в 332 футпринта, 36–40 мс на плате в 414 — а инструменты, которые на
футпринты вообще не смотрят, платят 0.7–10 мс. Число — факт про цену, а не
политика: TTL не введён.
