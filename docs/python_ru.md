...
вместе с) CLI/конфиг-воркфлоу — ни один не меняет формат `.sexp`-конфига или пайплайн `apply`/`extract`,
оба — тонкие обёртки над уже существующим. Про сам синтаксис `.sexp` —
[docs/config_ru.md](config_ru.md); про команды CLI — [docs/commands_ru.md](commands_ru.md).
...
## `kicadstamp.author` — кодинг расстановки вместо копипаста `.sexp`
...
собирай их напрямую, реальными Python-переменными вместо подстановки `{placeholder}` в конфиге:
...
# прямо в живой пайплайн apply, минуя шаг с генерируемым .sexp вовсе:
...
## Как сделать нецепейьно: пропустить шаг с генерируемым .sexp
...
`.sexp` → загрузить настоящий корневой конфиг (который его `include:`-ит) → применить. `cli_main` уже
делает ровно это.
...
`.sexp` руками: не тот ключ `nets:`, задвоенная строка `anchor_pad:`, имя листа, скопированное у соседнего
...
### Шаг 4 — применить по-настоящему, держать сгенерированный .sexp в git
...
Это общий паттерн для всего, что зависит от геометрии и не выводится безопасно из чисел в конфиге
одной арифметикой (сдвиг центр-компонента-к-паду, размеры футпринта) — измеряй против реальной платы через
...
- [docs/config_ru.md](config_ru.md) — `.sexp`-схема, которую эти Python-объекты отражают поле в поле.
