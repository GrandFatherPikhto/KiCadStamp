...
def entity_anchor_identity(entity: Entity, cfg: Any) -> tuple[str, str | None, str | None] | None:
    ...
    self-anchor node. An imprint-based Entity (design_2026_09_05_scheme_
    ...
    if cfg is None:
        return None
    if entity.imprint is not None:
        # An imprint Entity can never be a role-anchor subject: it has no
...
def entity_cell_has_copper(entity: Entity, cfg: Any) -> bool:
    ...
    if cfg is None:
        return False
    if entity.imprint is not None:
        # An imprint-based Entity carries its literal copper in the recorded
        # imprints: snapshot (imprint_capture.py), never in a cell — it
