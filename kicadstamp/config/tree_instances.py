...
def expand_tree_instances(data: dict) -> dict:
    ...
    # Э2.2 (plan_2026_09_13_tree_duplicate_name_diagnosis): two declarations with
    # the SAME name materialize two trees of that name (and two sets of entity
    # copies) — strictly fatal, and it must be said here, by the thing that
    # caused it. Left to the trees/entities duplicate-name checks downstream it
    # arrived as a symptom instead of a diagnosis: both generated trees carry the
    # same rewritten node refs, so the loader's per-node walk reported rule 2's
    # "a record's position source must be exactly one" under a tree name the user
    # never typed twice (the live report this guard comes from).
    # Raw-dict validation, duplicated on purpose — this function runs BEFORE
    # entries.py (see the module docstring), same discipline as cluster/params/
    # anchor/rotation below. Entries that are not mappings, or whose name is not
    # a string, are skipped: the per-entry guards below have the better message.
    declared_names: dict[str, int] = {}
    for inst in instances:
        if isinstance(inst, dict) and isinstance(inst.get('name'), str):
            declared_names[inst['name']] = declared_names.get(inst['name'], 0) + 1
    duplicate_decl_names = sorted(
        name for name, count in declared_names.items() if count > 1)
    if duplicate_decl_names:
        raise ValidationError(format_fatal_error(
            _("duplicate name(s) in tree_instances: {names}").format(
                names=duplicate_decl_names),
            [_("every tree_instances: declaration needs a unique name — each one "
               "materializes exactly one tree (plus its entity copies), so give "
               "the second declaration its own name:, or drop it")]))
    ...
    for idx, inst in enumerate(instances):
        ...
        # Э2.1 (same plan): the instance's name must be FREE — it is the name of
        # the tree this declaration materializes. trees_by_name is the index of
        # every tree that exists at this point (literal trees: entries, the
        # templates, and the trees generated by earlier declarations — it is
        # updated at the bottom of this loop), so a collision with any of the
        # three is caught HERE, before anything is built. Without this guard the
        # collision surfaced far away and mislabelled: two same-named trees share
        # their rewritten node refs, and the loader then reported rule 2's
        # "a record's position source must be exactly one" (the live report).
        if instance_name in trees_by_name:
            raise ValidationError(format_fatal_error(
                _("duplicate tree name from tree_instance {name!r}").format(
                    name=instance_name),
                [_("every tree in one config needs a unique name, and a tree "
                   "with that name already exists here — a trees: entry, the "
                   "template itself, or an earlier tree_instances: declaration — "
                   "rename this instance, or rename/remove the tree that already "
                   "carries the name")]))
